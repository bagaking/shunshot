...
const createCaptureWindow = async () => {
  ...
  // 计算所有显示器的工作区域总边界
  const workArea = {
    x: Math.min(...displays.map(d => d.workArea.x)),
    y: Math.min(...displays.map(d => d.workArea.y)),
    width: Math.max(...displays.map(d => d.workArea.x + d.workArea.width)) - Math.min(...displays.map(d => d.workArea.x)),
    height: Math.max(...displays.map(d => d.workArea.y + d.workArea.height)) - Math.min(...displays.map(d => d.workArea.y))
  }
  ...
  try {
    ...
    const sources = await desktopCapturer.getSources({
      types: ['screen'],
      thumbnailSize: {
        width: Math.round(workArea.width * primaryDisplay.scaleFactor),
        height: Math.round(workArea.height * primaryDisplay.scaleFactor)
      }
    })
    ...
    captureWindow = new BrowserWindow({
      ...
      type: 'panel', // 使用 panel 类型窗口以获得更好的显示效果
      webPreferences: {
