...
const MainContent: React.FC = () => {
  ...
  useEffect(() => {
    window.electronAPI.setWindowSize(isMinimized ? 40 : 280, isMinimized ? 40 : 380)
  }, [isMinimized])
  ...
  if (isMinimized) {
    return (
      <div 
        className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 
                   shadow-lg shadow-blue-500/20 cursor-pointer flex items-center justify-center text-white
                   transition-all duration-200 ease-out hover:scale-105 active:scale-95
                   [&_*]:select-none [-webkit-app-region:drag] relative group"
  ...
  return (
    <div className="bg-white/90 backdrop-blur-lg rounded-2xl overflow-hidden
                    ...
                    w-[280px] transition-all duration-200 ease-out
                    [&_*]:select-none">
      {/* 标题栏 - 支持拖拽 */}
      <div className="flex items-center justify-between p-3 bg-gradient-to-r from-gray-50/50 to-white/50 [-webkit-app-region:drag]">
        ...
        <div className="flex items-center space-x-1">
          ...
          >
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 px-2 py-1 bg-gray-800 text-white text-xs rounded
                          opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none">
              最小化
            </div>
          </button>
